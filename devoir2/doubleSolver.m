function [i1, i2, X] = doubleSolver (Img, A1, B1, C1, A2, B2, C2, iterator, dir, a, k)
% Solve
...
save_image(Img, sprintf('%s/original_%d', dir, a*100));
...
save_image(IB, sprintf('Q2/blurred_%d', a*100));

Delta = randn(M, N); % Ok mean = 0 variance = 1
IBN = (IB + Delta); % blurred image with perturbation
save_image(IBN, sprintf('Q2/noise_%d', a*100));
